...
export default {
	...
	methods: {
		rolePermission() {
			axios.get("/role-permissions").then(response => {
				response.data.forEach(element => {
					if (element.name == "package-size.create") {
						this.canAdd = true;
					} else if (element.name == "package-size.edit") {
						this.canEdit = true;
					} else if (element.name == "package-size.delete") {
						this.canDelete = true;
					}
				});
			});
		...
		getPackageSizes(page = 1) {
			axios.get("All-Package-Size?page=" + page).then(response => {
